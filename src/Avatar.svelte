<svelte:options tag="rendikit-avatar" />

<script lang="typescript">
  import cssVars from 'svelte-css-vars'
  import { Color } from 'rendikit-theme'

  export let appereance = 'circle'
  export let size = 32
  export let src = ''

  $: styleMain = {
    white: Color.Normal('white'),
    'neutral-200': Color.Normal('N200'),
    width: `${size}px`,
    height: `${size}px`,
    'border-radius': appereance === 'circle' ? `50%` : `8px`,
    'background-image': `url(${src})`
  }
</script>

<span use:cssVars={styleMain}>
  {#if src === ''}
    <span class="empty">
      <span>
        <svg
